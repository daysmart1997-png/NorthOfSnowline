extends Node3D
# Visible evidence for the opening's closed road, inside the existing map edge.
func build(world)->void:
	name="SouthRoadClosure"
	...
	for i in range(clusters.size()):
		...
		var at:=Vector3(position_data.x,world.terrain_height(position_data.x,position_data.z)-.10,position_data.z)
	...
	for x in [-4.5,4.5]:
		for z in [39.8,42.2,44.6]:
			var y:float=world.terrain_height(x,z)
			world.box(Vector3(x,y+.56,z),Vector3(.11,1.12,.12),"494e50",true,self)
		var rail=world.box(Vector3(x,world.terrain_height(x,42)+.86,42),Vector3(.13,.20,4.6),"606b70",false,self)
		if x<0:rail.rotation.x=.10
	for x in [-2.7,2.7]:world.box(Vector3(x,world.terrain_height(x,44)+.45,44),Vector3(.13,.90,.16),"514b40",true,self)
	world.box(Vector3(0,world.terrain_height(0,44)+.70,44),Vector3(5.5,.20,.15),"79604b",true,self)
