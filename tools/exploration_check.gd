...
func check_exploration()->void:
	...
	assert(cassette.speaker.bus=="SnowMusic" and cassette.wind.bus=="SnowWind" and AudioServer.get_bus_send(AudioServer.get_bus_index(cassette.wind.bus))=="SnowEffects","Wind low-pass sub-bus still inherits the effects volume control")
	assert(is_equal_approx(AudioServer.get_bus_volume_db(AudioServer.get_bus_index("SnowEffects")),linear_to_db(.4)))
